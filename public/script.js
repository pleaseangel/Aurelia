...
const firebaseConfig = {
    ...
    projectId: "aurelia-79f90", // FIXED: Removed extra "y"
...
async function generatePrayer() {
    try {
        ...
        const prayerData = await callPrayerAPI(userProfile);
        
        currentPrayer = {
            prayer: prayerData.prayerText,
            audio: prayerData.audioData,
            profile: userProfile,
...
// Call prayer generation API - FIXED: Send data format that matches backend
async function callPrayerAPI(profile) {
    try {
        ...
        return {
            prayerText: generateFallbackPrayer(profile),
            audioData: null
        };
    }
}
...
function displayPrayer(prayerData) {
    ...
    // Auto-speak prayer if supported and audio is available
    if (prayerData.audio) {
        setTimeout(() => speakPrayerAudio(prayerData.audio), 1000);
    } else if ('speechSynthesis' in window) {
        setTimeout(() => speakPrayer(), 1000);
    }
}

// Speak prayer using generated audio
function speakPrayerAudio(audioData) {
    try {
...
// Speak prayer using text-to-speech (fallback)
